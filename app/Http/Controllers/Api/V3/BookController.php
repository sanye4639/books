...
class BookController extends Controller{
    ...
    */
    public function index(){
        ...
        if($type == 'rank'){
            /*排行榜列表*/
            $data = Books::select(['book_list3.*'])
                ->leftJoin('visitor as v',function ($join)use($param){
                    $join->on('v.book_id','=','book_list3.id')
                ...
                })->groupBy('book_list3.id')
                ->orderByRaw(DB::raw("sum(v.visitor_num) desc"))
                ->orderBy('book_list3.sort','desc')
    ...
    */
    public function search(){
        ...
        })->where('dstatus','1')->orderBy('sort','desc')->orderBy('rn','asc')->limit(50)->get();
        if(count($data)>0){
    ...
    */
    public function detail(){
        ...
        /*最新章节列表*/
        $chapter_tableName = 'booksChapter_v3.book_chapter_'.$book_id%100;
        $data['new_chapterList'] = DB::table($chapter_tableName)
            ->select(['id','book_id','title','oss_url','created_at'])
            ->whereBook_id($book_id)
            ->orderBy('sort','desc')->limit(5)->get();

        if(count($data['new_chapterList']) == 0){
            return echojson('该小说暂无章节录入<br>请联系管理员',0,$data);
        }
        /*最新章节更新时间*/
        $data['chapterUpdateTime'] = $data['new_chapterList'][0]->created_at;
    ...
    */
    public function bookContent(){
        ...
        $data = Books::select(['id','name','over'])->find($id);
        $chapter_tableName = 'booksChapter_v3.book_chapter_'.$id%100;
        ...
        $data['content'] = preg_replace('/###([^^]*?)+###/','',$data['content'],1);
        /*获取上一页下一页的章节ID*/
    ...
    */
    function chapterList(){
        $book_id = Input::get('id');
        $chapterUpdateTime = Input::get('chapterUpdateTime');
        /*最新章节列表*/
        $chapter_tableName = 'booksChapter_v3.book_chapter_'.$book_id%100;
        /*章节列表*/
        $data['chapterList'] = object_array(DB::table($chapter_tableName)
            ->select(['id','book_id','title','created_at'])
            ->whereBook_id($book_id)
            ->orderBy('sort','asc')
            ->get());
        $data['chapterUpdateTime'] = end($data['chapterList'])['created_at'];
        if($data['chapterUpdateTime'] == $chapterUpdateTime){
            return echojson('ok',2);
        }else{
            return echojson('ok',1,$data);
        }
    }
