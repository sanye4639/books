...
class BookController extends Controller{
    ...
    */
    public function index(){
        ...
        if($type == 'rank'){
            /*排行榜列表*/
            $data = Books::select(['book_list2.*'])
                ->leftJoin('visitor as v',function ($join)use($param){
                    $join->on('v.book_id','=','book_list2.id')
                ...
                })->groupBy('book_list2.id')
                ->orderByRaw(DB::raw("sum(v.visitor_num) desc"))
                ->orderBy('book_list2.sort','desc')
    ...
    */
    public function detail(){
        ...
        $data['typeCn'] = get_bookType2($data->type);
        $data['overCn'] = ($data->over == 1)?'连载':'完结';
        /*最新章节列表*/
        $chapter_tableName = 'booksChapter_v2.book_chapter_'.$book_id%100;
    ...
    */
    public function bookContent(){
        ...
        $chapter_tableName = 'booksChapter_v2.book_chapter_'.$id%100;

        $chapter_data = DB::table($chapter_tableName)->select(['id','title','content','sort'])->whereId($chapter_id)->first();
        ...
        $data['content'] = $chapter_data['content'];
        /*获取上一页下一页的章节ID*/
