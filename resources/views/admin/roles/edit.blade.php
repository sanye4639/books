                            ...
                            <div class="am-form-group">
                                ...
                                <div class='am-u-sm-9' id="table">
                                    <label class="am-checkbox-inline">
                                        {{Form::checkbox('permissions[]',  1, $role->permissions,array('data-am-ucheck','class'=>'allChoose') ) }}
                                        <b>超级权限</b>
                                    </label>
                                    @foreach ($menu_data as $val)
                                        <div class="table">
                                            <label class="am-checkbox-inline">
                                                {{Form::checkbox('permissions[]',  $val->permission_id, $role->permissions,array('data-am-ucheck','class'=>'allChoose') ) }}
                                                <b>{{$val->menu_name}}</b>
                                            </label>
                                            <br>
                                                ...
                                                <div style="margin-left: 20px" class="children_check">
                                                    @foreach($val->children as $v)
                                                        <label class="am-checkbox-inline am-success">
                                                            {{Form::checkbox('permissions[]',  $v->permission_id, $role->permissions,array('data-am-ucheck') ) }}
                                                            {{$v->menu_name}}
                                                        </label>
                                                    @endforeach
                                                </div>
    ...
    <script>
        $(function () {
            ...
            $("#table .table").each(function () {
                var chknum = $(this).find(".children_check input:checkbox").size();//选项总个数
                if(chknum>0){ //排除没有下级菜单元素
                    var chk = 0;
                    $(this).find(".children_check  input:checkbox").each(function () {
                    ...
                    //默认上级选中
                    if(chknum==chk){
                        $(this).find(".allChoose").prop("checked",true);
                    }else{
                        $(this).find(".allChoose").prop("checked",false);
                    }
                }
            });
        })
